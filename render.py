...
async def main():
    ...
    proc=subprocess.Popen([CHROME,"--headless=new",f"--remote-debugging-port={PORT}",f"--user-data-dir={prof}",f"--window-size={W},{H}","--hide-scrollbars","--mute-audio","--no-first-run","--enable-gpu","--ignore-gpu-blocklist","--use-angle=metal","about:blank"],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    try:
        for _ in range(50):
        ...
        async with websockets.connect(ws_url,max_size=None) as ws:
            ...
            await call("Emulation.setDeviceMetricsOverride",{"width":W,"height":H,"deviceScaleFactor":1.5,"mobile":False})
            ...
            for i in range(N):
                ...
                shot=await call("Page.captureScreenshot",{"format":"jpeg","quality":96},timeout=30)
                open(f"{OUT}/f{i:04d}.jpg","wb").write(base64.b64decode(shot["data"]))
